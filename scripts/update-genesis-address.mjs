#!/usr/bin/env node
/**
 * Post-deploy helper: wire Genesis contract address into .env and mint config.
 * Usage: node scripts/update-genesis-address.mjs 0xYourContractAddress
 */
...
if (!address || !/^0x[a-fA-F0-9]{40}$/.test(address)) {
  console.error("Usage: node scripts/update-genesis-address.mjs 0xContractAddress");
  process.exit(1);
}
...
mintText = mintText.replace(
  /const MAINNET_DEPLOYED_CONTRACT_ADDRESS: `0x\$\{string\}` \| null = [^;]+;/,
  `const MAINNET_DEPLOYED_CONTRACT_ADDRESS: \`0x\${string}\` | null = "${address}";`,
);
